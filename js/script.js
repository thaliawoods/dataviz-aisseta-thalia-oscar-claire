...
const sendLabelToOpenAI = (label) => {
    ...
    const songPrompt = `suggest me a modern song both in french and english who matches with the recipe mood following this syntax : Artist - Song"${label} "`;
    fetch(openaiEndpoint, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
      ...
      .then((data) => {
        if (data) {
          ...
          fill="none" class="svg-icon">
              ...
              <a href=${data.youtubeLink} class="link">Listen here</a>
          </button>`
